...
class TestDoctorIntegration:
    ...
    def _isolate(self, tmp_path: Path, monkeypatch):
        ...
        # Patch the module-level variable — load_settings() uses it directly
        monkeypatch.setattr(_settings, "_CONFIG_PATH", config)

        # Also patch _get_config_path in core (imports _CONFIG_PATH at call time,
        # so the monkeypatch above already covers it, but belt-and-suspenders):
    ...
    def test_doctor_runs_all_categories(self, runner: CliRunner):
        ...
        with (
            patch("studyctl.doctor.updates._fetch_pypi_version", return_value=None),
            patch("studyctl.doctor.updates._read_cache", return_value={}),
            patch("studyctl.doctor.agents._fetch_manifest", return_value=None),
            patch("studyctl.doctor.agents._detect_ai_tools", return_value=[]),
        ):
    ...
    def test_doctor_json_has_required_fields(self, runner: CliRunner):
        ...
        data = json.loads(result.output)
        for item in data:
            # name and message are also part of the CheckResult schema
            assert "name" in item
    ...
    def test_doctor_category_filter(self, runner: CliRunner):
        ...
        with (
            ...
            result = runner.invoke(doctor, ["--json", "--category", "core"])
    ...
    def test_update_json_output_is_list(self, runner: CliRunner):
        ...
        data = json.loads(result.output)
        assert isinstance(data, list)
        # update command only returns core + updates categories
        for item in data:
    ...
    def test_doctor_quiet_returns_summary_line(self, runner: CliRunner):
        ...
        with (
            ...
            result = runner.invoke(doctor, ["--quiet"])

        assert result.exit_code in (0, 1, 2)
        output = result.output.strip()
        # Summary line always ends with a period
